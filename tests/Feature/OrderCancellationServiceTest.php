...
test('can cancel unpaid order in progress', function (): void {
    ...
    $service = resolve(OrderCancellationService::class);
...
test('cannot cancel order without correct PIN', function (): void {
    $user = User::factory()->create(['admin_pin' => '1234']);
    $order = Order::factory()->create([
        'status' => 'pending',
        'payment_status' => 'unpaid',
...
test('cannot cancel order that is not pending and unpaid', function (): void {
    $user = User::factory()->create(['admin_pin' => '1234']);
    $order = Order::factory()->create([
        'status' => 'completed',
        'payment_status' => 'paid',
...
test('cancellation creates history record', function (): void {
    $user = User::factory()->create(['admin_pin' => '1234']);
    $order = Order::factory()->create([
        'status' => 'pending',
        'payment_status' => 'unpaid',
...
test('can check if order can be cancelled', function (): void {
