...
final class IngredientForm
{
                ...
                Section::make('Basic Information')
                    ...
                    ->schema([
                        ...
                        Textarea::make('description')
                            ->label('Description')
                            ->placeholder('e.g., Premium Arabica beans from Colombia, medium roast')
                            ->helperText('Detailed description of the ingredient')
                            ->rows(3)
                            ->columnSpanFull(),
                    ]),
                        ...
                        Grid::make(2)
                            ->schema([
                                Toggle::make('is_trackable')
                                    ...
                                    ->helperText('Enable real-time stock tracking for this ingredient')
                                    ->default(true)
                                    ->reactive()
                                    ->afterStateUpdated(fn ($state, callable $set) => self::updateStockFieldsVisibility($state, $set))
                                    ->columnSpan(1),
                        ...
                        Grid::make(2)
                            ->schema([
                                ...
                                Placeholder::make('stock_status')
                                    ->label('Stock Status')
                                    ->content(fn ($record) => self::getStockStatus($record))
                                    ->columnSpan(1),
                            ]),
                    ]),
            ])
    ...
    private static function updateStockFieldsVisibility(bool $isTrackable, callable $set): void
    {
